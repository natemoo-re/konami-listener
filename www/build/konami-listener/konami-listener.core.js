...
(function(window, document, Context, appNamespace, publicPath) {
  ...
  function initElementListeners(plt, elm) {
    ...
    cmpMeta.listenersMeta && // we've got listens
    cmpMeta.listenersMeta.forEach(listenMeta => {
      // go through each listener
      listenMeta.eventDisabled || // only add ones that are not already disabled
    ...
    if (instance) {
      ...
      if (cmpMeta && cmpMeta.listenersMeta) {
        ...
        if (shouldEnable) {
          ...
          listenMeta && // found the listen meta, so let's add the listener
          plt.domApi.$addEventListener(elm, eventName, ev => instance[listenMeta.eventMethodName](ev), listenMeta.eventCapture, listenMeta.eventPassive, attachTo);
        } else {
